 ...
 */
 ...
 (function ( $ ) {
		...
		var draw = function() {
          ...
          for (var i = 0; i < nDates; i++) {
            ...
            for (var j = 0; j < nDate; j++) {
			  ...
			  if (typeof settings.notes !== 'undefined') {
                if (dateIsdias_livres(new Date(year, month - 1, date))) {
                  colDateClass += " dia_livre ";
                }
              }

			  if (typeof settings.notes !== 'undefined') {
                if (dateIsdias_ocupados(new Date(year, month - 1, date))) {
                  colDateClass += " dia_ocupado ";
                }
              }
	...
	var dateIsdias_livres = function(date) {
          var notesLength = settings.dias_livres.length;
          for (var i = 0; i < notesLength; i++) {
                var datenote = settings.dias_livres[i].date.split('-');
                var nDate = new Date(datenote[0], datenote[1] - 1, datenote[2]);
	...
	var dateIsdias_ocupados = function(date) {
          var notesLength = settings.dias_ocupados.length;
          for (var i = 0; i < notesLength; i++) {
                var datenote = settings.dias_ocupados[i].date.split('-');
                var nDate = new Date(datenote[0], datenote[1] - 1, datenote[2]);
